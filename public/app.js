...
function onLLMFieldChange() {
  var cfg = { model: document.getElementById('llm-model').value.trim(), endpoint: document.getElementById('llm-endpoint').value.trim(), apiKey: document.getElementById('llm-apikey').value.trim() };
  localStorage.setItem('jd-llm-config', JSON.stringify(cfg));
  updateLLMHint(cfg);
}
